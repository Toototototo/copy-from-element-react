...
var TableStore = function (_Component) {
  ...
  TableStore.prototype.setHoverRow = function setHoverRow(index) {
    var isComplex = this.state.isComplex;
  ...
  TableStore.prototype.setCurrentRow = function setCurrentRow(row, index) {
    var _this3 = this;
  ...
  TableStore.prototype.toggleRowSelection = function toggleRowSelection(row, isSelected) {
    ...
    if (Array.isArray(currentRowKey)) {
      ...
      this.dispatchEvent("onRowChange", newCurrentRowKey, newCurrentRowKey.map(function (key) {
        return data.find(function (d) {
          return getRowIdentity(d, rowKey) === key;
        });
      }));
      this.dispatchEvent("onRowSelect", row, isSelected, newCurrentRowKey.map(function (key) {
        return data.find(function (d) {
          return getRowIdentity(d, rowKey) === key;
        });
      }));
    ...
    this.setState(function (state) {
      ...
      var selectedRows = _this4.state.selectedRows;

      _this4.dispatchEvent("onSelect", selectedRows, row);
      _this4.dispatchEvent("onSelectChange", selectedRows);
      _this4.dispatchEvent("onRowChange", selectedRows.map(function (row) {
        return getRowIdentity(row, rowKey);
      }), selectedRows);
    });
  };

  TableStore.prototype.toggleAllSelection = function toggleAllSelection(isSelected) {
    var _this5 = this;
    ...
    if (Array.isArray(currentRowKey)) {
      ...
      allSelectableRows.forEach(function (row) {
        if (isSelected) {
      ...
      this.dispatchEvent("onRowSelectAll", isSelected, newCurrentRowKey.map(function (key) {
        return data.find(function (d) {
          return getRowIdentity(d, rowKey) === key;
        });
      }), changeRows);
  ...
  TableStore.prototype.changeSortCondition = function changeSortCondition(column, order) {
    ...
    if (!order || sortable === "custom") {
      ...
      if (sortMethod) {
        ...
        sortedData = data.sort(function (a, b) {
          var aVal = getValueByPath(a, dataIndex);
          var bVal = getValueByPath(b, dataIndex);
  ...
  TableStore.prototype.render = function render() {
    ...
    var renderExpanded = (columns.find(function (column) {
  ...
  _createClass(TableStore, [{
    ...
    get: function get() {
      ...
      if (Array.isArray(currentRowKey)) {
        return currentRowKey && currentRowKey.length > 0 && currentRowKey.length < selectableData.length;
      }

      return selectedRows && selectedRows.length > 0 && selectedRows.length < selectableData.length;
    }
  }, {
    key: "isAllSelected",
    get: function get() {
      var _props5 = this.props,
          currentRowKey = _props5.currentRowKey,
